...
/**
 * Skeleton_Plugin_Settings Class
 *
 * @class Skeleton_Plugin_Settings
 * @version	1.0.0
 * @since 1.0.0
 * @package	Skeleton_Plugin
 * @author Nate
 */
final class Skeleton_Plugin_Settings {
    /**
    ...
    private $_has_select;
    /**
     * Main Skeleton_Plugin_Settings Instance
     *
     * Ensures only one instance of Skeleton_Plugin_Settings is loaded or can be loaded.
     *
     * @since 1.0.0
     * @static
     * @return Main Skeleton_Plugin_Settings instance
     */
        ...
        if ( is_array( $input ) && 0 < count( $input ) ) {
            ...
            foreach ( $input as $k => $v ) {
                ...
                if ( ! method_exists( $this, $method ) ) {
                    if ( true === (bool)apply_filters( 'skeleton-plugin-validate-field-' . $fields[$k]['type'] . '_use_default', true ) ) {
                ...
                if ( '' != $method ) {
                    add_filter( 'skeleton-plugin-validate-field-' . $fields[$k]['type'], array( $this, $method ) );
                }
                $method_output = apply_filters( 'skeleton-plugin-validate-field-' . $fields[$k]['type'], $v, $fields[$k] );
    ...
    public function render_field ( $args ) {
        $html = '';
        if ( ! in_array( $args['type'], $this->get_supported_fields() ) ) return '';
        ...
        $key 				= Skeleton_Plugin()->token . '-' . $args['section'] . '[' . $args['id'] . ']';
        $method_output 		= $this->$method( $key, $args );
        if ( ! is_wp_error( $method_output ) ) {
            $html .= $method_output;
        }

        if ( isset( $args['type'] ) && ! in_array( $args['type'], (array)apply_filters( 'skeleton-plugin-no-description-fields', array( 'checkbox' ) ) ) ) {
            if ( isset( $args['description'] ) ) {
                $description = '<p class="description">' . wp_kses_post( $args['description'] ) . '</p>' . "\n";
                if ( in_array( $args['type'], (array)apply_filters( 'skeleton-plugin-new-line-description-fields', array( 'textarea', 'select' ) ) ) ) {
    ...
    public function get_settings_sections () {
        ...
        $settings_sections['standard-fields'] = __( 'Standard Fields', 'skeleton-plugin' );
        $settings_sections['special-fields'] = __( 'Special Fields', 'skeleton-plugin' );

        return (array)apply_filters( 'skeleton-plugin-settings-sections', $settings_sections );
    }
    ...
    public function get_settings_fields ( $section ) {
        ...
        switch ( $section ) {
            case 'standard-fields':
                $settings_fields['text'] = array(
                    'name' => __( 'Example Text Input', 'skeleton-plugin' ),
                    'type' => 'text',
                    'default' => '',
                    'section' => 'standard-fields',
                    'description' => __( 'Place the field description text here.', 'skeleton-plugin' )
                );
                $settings_fields['textarea'] = array(
                    'name' => __( 'Example Textarea', 'skeleton-plugin' ),
                    'type' => 'textarea',
                    'default' => '',
                    'section' => 'standard-fields',
                    'description' => __( 'Place the field description text here.', 'skeleton-plugin' )
                ...
                $settings_fields['checkbox'] = array(
                    'name' => __( 'Example Checkbox', 'skeleton-plugin' ),
                    'type' => 'checkbox',
                    'default' => '',
                    'section' => 'standard-fields',
                    'description' => __( 'Place the field description text here.', 'skeleton-plugin' )
                ...
                $settings_fields['radio'] = array(
                    'name' => __( 'Example Radio Buttons', 'skeleton-plugin' ),
                    'type' => 'radio',
                    'default' => '',
                    'section' => 'standard-fields',
                    'options' => array(
                        'one' => __( 'One', 'skeleton-plugin' ),
                        'two' => __( 'Two', 'skeleton-plugin' ),
                        'three' => __( 'Three', 'skeleton-plugin' )
                    ),
                    'description' => __( 'Place the field description text here.', 'skeleton-plugin' )
                );
                $settings_fields['select'] = array(
                    'name' => __( 'Example Select', 'skeleton-plugin' ),
                    'type' => 'select',
                    'default' => '',
                    'section' => 'standard-fields',
                    'options' => array(
            ...
            case 'special-fields':
                $settings_fields['select_taxonomy'] = array(
                    'name' => __( 'Example Taxonomy Selector', 'skeleton-plugin' ),
        ...
        return (array)apply_filters( 'skeleton-plugin-settings-fields', $settings_fields );
    }
    ...
    public function get_supported_fields () {
        return (array)apply_filters( 'skeleton-plugin-supported-fields', array( 'text', 'checkbox', 'radio', 'textarea', 'select', 'select_taxonomy' ) );
    }
    /**
    ...
    public function get_value ( $key, $default, $section ) {
        $values = get_option( 'skeleton-plugin-' . $section, array() );
    ...
    public function get_settings ( $section = '' ) {
        ...
        if ( 0 < count( $sections ) ) {
            foreach ( $sections as $k => $v ) {
                ...
                $values = get_option( 'skeleton-plugin-' . $v, array() );
